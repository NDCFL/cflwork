...
$('#mytab').bootstrapTable({
    ...
    url: "/contracmasterMsg/contracmasterMsgList",//要请求数据的文件路径
    ...
    columns: [
        ...
        },
        {
            ...
            field: '',
            formatter: function (value, row, index) {
                var e = '<a title="编辑"  id="contracmasterMsg" href="/contracmasterMsg/update/'+row.id+'"><i class="glyphicon glyphicon-pencil" alt="修改" style="color:green">修改</i></a> ';
...
$('#search_btn').click(function () {
    $('#mytab').bootstrapTable(
        'refresh',
        {
            url: '/contracmasterMsg/findContracmasterMsgList',
            query: {
                id: $("#id__").val(),
                masterId: $("#masterId__").val(),
                sendId: $("#sendId__").val(),
                name: $("#name__").val(),
                content: $("#content__").val(),
                createTime: $("#createTime__").val(),
...
function del(id, status) {
    ...
    layer.confirm('确认要删除吗？', function (index) {
        $.ajax({
            type: 'POST',
            url: '/contracmasterMsg/deleteContracmasterMsg/' + id,
            dataType: 'json',
...
function edit(name) {
    $.post("/contracmasterMsg/findContracmasterMsg/" + name,
        function (data) {
...
function updatestatus(id, status) {
    $.post("/contracmasterMsg/updateStatus/" + id + "/" + status,
        function (data) {
            if (status == 0) {
                if (data.message == "ok") {
...
//查询按钮事件
$('#search_btn').click(function () {
    $('#mytab').bootstrapTable('refresh', {url: '/contracmasterMsg/contracmasterMsgList'});
})
...
$("#update").click(function () {
    $.post(
        "/contracmasterMsg/contracmasterMsgUpdateSave",
...
$("#add").click(function () {
    $.post(
        "/contracmasterMsg/contracmasterMsgAddSave",
        $("#formadd").serialize(),
        function (data) {
...
function deleteMany11() {
    ...
    layer.confirm('确认要执行批量删除网站信息数据吗？', function (index) {
        $.post(
            "/contracmasterMsg/deleteManyContracmasterMsg",
            {
                "manyId": $("#deleteId").val()
            },
            function (data) {
